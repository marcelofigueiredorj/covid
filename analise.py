...
def carrega_dados_brasil(df):
    dados_geral_brasil = df[df['estado'].isna()]
    return dados_geral_brasil

@st.cache_resource
def carrega_dados_gerais_brasil(df):
    dados_geral_brasil = (df[df['municipio'].isna()])
    dados_geral_brasil = dados_geral_brasil.groupby(
        'regiao').sum().sort_values('casosAcumulado', ascending=False)
...
def finalizcao():
    ...
    st.markdown('- Desenvolvido por: Marcelo Figueiredo 🧑🏻‍💻 <br> \
                - Me dê um alô [Whatsapp](https://api.whatsapp.com/send?phone=5521987912190&text=Ol%C3%A1,%20poderiamos%20de%20conversar)',
                unsafe_allow_html=True)
...
def carrega_pagina():
    ...
    apresentacao()
    recuperados, acompanhamento = extrai_dados_casos_recuperados(
        dados[dados['estado'].isna()])
    ...
    if st.sidebar.checkbox('Dados Gerais Brasil'):
        ...
        indices_br = ['casosNovos', 'casosAcumulado',
    ...
    if st.sidebar.checkbox('Visualizar por Região'):
        titulo = 'Escolha o índice que deseja visualizar:'
        indices = ['casosAcumulado', 'casosNovos',
                   'obitosAcumulado', 'obitosNovos']
        ...
        df_regiao = dados[dados['municipio'].isna() & (
            dados['regiao'] == filtro_regiao)]
        df_regiao = df_regiao.groupby('estado').sum(
        ).sort_values(indice_regiao, ascending=False)
        grafico_casos_acumulados = grafico_barras(
            df_regiao, f'Dados por estado da região {filtro_regiao}', indice_regiao)
